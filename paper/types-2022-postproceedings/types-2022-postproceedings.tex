...
Indeed, the handles that Supervisionary uses to reference kernel objects under its management may be thought of as a form of \emph{capability}, in the security sense.
However, unlike typical capabilities, Supervisionary's capabilities are extremely expressive, essentially capturing the full expressive power of HOL, and can potentially be used to enforce fine-grained correctness and security properties at runtime.
\end{abstract}
...
\begin{center}
\begin{tabular}{lll}
\emph{Name}                      & \emph{Arguments} & \emph{Description} \\
\hline
\texttt{TypeFormer.Register}     & Arity            & Allocates a fresh handle and returns it \\
\texttt{TypeFormer.Resolve}      & Handle           & An arity if handle points-to a registered \\
                                 &                  & type-former, error otherwise \\
\texttt{TypeFormer.IsRegistered} & Handle           & Boolean indicating if the handle points-to \\
                                 &                  & a registered type-former or not \\
\end{tabular}
\end{center}
\caption{Table of Supervisionary system calls for type-former management}
\label{fig.type-former.system-calls}
\end{figure}
...
\emph{Name}                        & \emph{Arguments} & \emph{Description} \\
\hline
\texttt{Type.Register.Variable}    & \\
\texttt{Type.Register.Combination} & \\
\texttt{Type.Register.Function}    & \\
\texttt{Type.Resolve}              & \\
\texttt{Type.IsRegistered}         & \\
\texttt{Type.Split.Variable}       & \\
\texttt{Type.Test.Variable}        & \\
\texttt{Type.Size}                 & \\
\texttt{Type.Variables}            & \\
\texttt{Type.Substitute}           & \\
\end{tabular}
\end{center}
\caption{(Excerpted) table of Supervisionary system calls for type management}
\label{fig.type.system-calls}
\end{figure}

Building atop the heap of type-formers is another heap: the heap of types.
Referring back to the grammar of HOL types introduced in Section~\ref{sect.implemented.logic}, all allocated entries within this heap are tagged either with a \texttt{V}, indicating that they are a type-variable, or with an \texttt{F}, indicating that they are a ``combination'' of a type-former applied to a list of argument types.
...
\emph{Name}                      & \emph{Arguments} & \emph{Description} \\
\hline
\texttt{Constant.Register}       & Handle, Handle   & \\
\texttt{Constant.Resolve}        & Handle           & \\
\texttt{Constant.IsRegistered}   & Handle           &
\end{tabular}
\end{center}
\caption{Table of Supervisionary system calls for constant management}
\label{fig.constant.system-calls}
\end{figure}

\subsection{The term heap}

\begin{figure}[t!]
\begin{center}
\begin{tabular}{lll}
\emph{Name}                         & \emph{Arguments} & \emph{Description} \\
\texttt{Term.Register.Variable}     & \\
\texttt{Term.Register.Constant}     & \\
\texttt{Term.Register.Application}  & \\
\texttt{Term.Register.Lambda}       & \\
\texttt{Term.IsRegistered}          & \\
\texttt{Term.Split.Application}     & \\
\texttt{Term.Test.Lambda}           & \\
\texttt{Term.Type.Infer}            & \\
\texttt{Term.Type.Variables}        & \\
\texttt{Term.Substitute}            &
\end{tabular}
\end{center}
\caption{(Excerpted) table of Supervisionary system calls for term management}
\label{fig.term.system-calls}
\end{figure}
...
\begin{tabular}{lll}
\emph{Name}                                   & \emph{Arguments} & \emph{Description} \\
\texttt{Theorem.Split.Hypotheses}             & \\
\texttt{Theorem.Register.Refl}                & \\
\texttt{Theorem.Register.Falsity.Elimination} & \\
\texttt{Theorem.Register.Forall.Introduction} &
\end{tabular}
\end{center}
\caption{(Excerpted) table of Supervisionary system calls for theorem management}
\label{fig.theorem.system-calls}
\end{figure}
