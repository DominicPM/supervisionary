...
\acknowledgements{We would like to thank Nick Spinale for many helpful conversations regarding Supervisionary, and Nathan Chong for comments on an early version of this paper.}
...
Whilst external tools, and automation written in other languages, \emph{can} interface with the kernel, it must do so indirectly, making use of a shim layer written in the system metalanguage.
...
Whilst this property is not unique to Supervisionary---values of the abstract type of theorems may also be passed around within any LCF-style system, for example---the objects which these handles denote could themselves be \emph{functions} of the runtime state of the program itself, or of the Supervisionary kernel.
...
The Supervisionary kernel is an extended exercise in pointer manipulation.
To specify the behaviour of Supervisionary's system calls we can therefore 
...
Supervisionary can be used like any other proof assistant, to statically establish properties of models of software or hardware systems, or reason within the rarefied domain of pure mathematics.
